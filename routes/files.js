...
router.get("/", authMiddleware, checkRol(["user", "admin"]), getFiles); // ruta renderizar la vista files mostrar todos los archivos
router.post("/search", authMiddleware, checkRol(["user", "admin"]), createFileValidator, getFileByName); // ruta para buscar archivos por titulo
